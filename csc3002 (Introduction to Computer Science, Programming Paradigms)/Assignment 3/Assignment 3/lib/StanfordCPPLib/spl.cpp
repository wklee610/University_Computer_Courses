...
/*static*/ GDimension GWindow::getScreenSize() {
    ...
    GThread::runOnQtGuiThread([&rec]() {
        rec = QGuiApplication::screens().first()->availableGeometry();
    });
...
void GObservable::fireGEvent(QWheelEvent* event,
    ...
    wheelEvent.setX(position.x());
    wheelEvent.setY(position.y());
...
double GContainer::getPadding() const {
    auto margins = _iqcontainer->contentsMargins();
    return (margins.left() + margins.top() + margins.right() + margins.bottom()) / 4.0;
}
